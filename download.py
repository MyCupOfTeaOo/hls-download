...
class Download():
    ...
    def write_log(self):
        with open(f'{self._path}/log.json', "w", encoding="utf-8") as f:
            f.write(json.dumps({
                ...
                "last_m3u8": self.name_filter_pattern.sub("", self._m3u8_url.split('/')[-1])
            }, ensure_ascii=False,
                indent=2, separators=(',', ':')))
    ...
    async def parse_list(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self._m3u8_url, headers=headers, timeout=TIMEOUT, proxy=self._proxy) as res:
                ...
                if key_res:
                    key = key_res.group()
                self.create_file(self.name_filter_pattern.sub("", self._m3u8_url.split(
                    '/')[-1]), list_text + '\n#EXT-X-ENDLIST')
                if key:
    ...
    def create_file(self, filename, text):
        ...
        filepath = f'{self._path}/{self.name_filter_pattern.sub("",filename.split("/")[-1])}'
        ...
        with open(path, encoding="utf-8", mode="r+") as f:
            ...
            if key_res:
                ...
                m3u8_file = m3u8_file.replace(
                    key_name, self.name_filter_pattern.sub("", self.file_suffix.sub(".ts", key_name.split("/")[-1])))
            ts_list = self.ts_pattern.findall(m3u8_file)
            for ts_link in ts_list:
                m3u8_file = m3u8_file.replace(
                    ts_link, self.name_filter_pattern.sub("", ts_link.split("/")[-1]))
            f.truncate(0)
            f.seek(0)
            f.write(m3u8_file)
